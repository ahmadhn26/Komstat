...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      # --- 👨‍💻 TAMBAHKAN BLOK KODE BARU INI TEPAT DI SINI ---
      tabItem(
        tabName = "tutorial", # Harus sama dengan tabName di menuItem
        fluidRow(
          box(
            ...
            div(
              ...
              # Judul dan deskripsi untuk video
              h4("Video tutorial berikut menjelaskan cara menggunakan dasbor ini secara efektif."),
              p("Silakan putar video untuk memulai."),
              br(),
              
              # Kode untuk menampilkan video dari folder 'www'
              tags$video(
                src = "tutorial.mp4", 
                type = "video/mp4",
                controls = "controls",
              ...
              p(style="font-style: italic; color: #555;",
                "Jika video tidak dapat diputar, pastikan file 'tutorial.mp4' sudah benar diletakkan di dalam folder 'www'."
              )
            )
          )
        )
      ) # <-- Ini adalah penutup untuk tabItem "tutorial" yang baru
